...
class PandasMixin:  
    ...
    @staticmethod
    def to_long(df: pd.DataFrame, index_name: str = None, variable_name: str = "VARIABLE", value_name: str = "VALUE") -> pd.DataFrame:
        """
    ...
    def csv_to_df(self, path: Path, header=0, index_col=None, case: str = "upper") -> pd.DataFrame:
...
class DataManager(PandasMixin):
    ...
    def merge_on_subject(self, table1: pd.DataFrame, table2: pd.DataFrame) -> pd.DataFrame:
        """Merge two tables on SUBJECT_ID."""
        return table1.merge(table2, how='inner', left_on=["SUBJECT_ID"], right_on=["SUBJECT_ID"])
    
    def merge_on_subject_admission(self, table1: pd.DataFrame, table2: pd.DataFrame) -> pd.DataFrame:
    ...
    def load_all_tables(self, data_path: Path) -> Dict[str, pd.DataFrame]:
        ...
        if "labevents" in dataframes and "ICUSTAY_ID" not in dataframes["labevents"].columns:
    ...
    def add_hcup_ccs_2015_groups(self, diagnoses: pd.DataFrame) -> pd.DataFrame:
        ...
        for dx in self._phenotype_definitions:
            for code in self._phenotype_definitions[dx]['codes']:
                def_map[code] = (dx, self._phenotype_definitions[dx]['use_in_benchmark'])
    ...
    def make_phenotype_label_matrix(self, phenotypes, stays=None) -> pd.DataFrame:
        ...
        if stays is not None:
            phenotypes = phenotypes.reindex(stays["ICUSTAY_ID"].sort_values(), fill_value=0)
    ...
    def variable_map(self) -> pd.DataFrame:
        ...
        var_map = self.csv_to_df(self.var_map_path, index_col=None).fillna('').astype(str)
        var_map = self.standardize_columns(var_map, case="upper")
        var_map["COUNT"] = var_map["COUNT"].astype(int)
        var_map = var_map.loc[(var_map[self.variable_column] != '') & (var_map["COUNT"] > 0)]
        var_map = var_map.loc[(var_map["STATUS"] == "ready")] 
        var_map["ITEMID"] = var_map["ITEMID"].astype(int)
        var_map = var_map[[self.variable_column, "ITEMID", "MIMIC_LABEL"]].set_index("ITEMID")
        self._var_map = var_map.rename(columns={self.variable_column: "VARIABLE", "MIMIC_LABEL": "MIMIC_LABEL"})
        return self._var_map
    
    @cached_property
    def variable_ranges(self) -> pd.DataFrame:
        columns = [self.variable_column, 'OUTLIER LOW', 'VALID LOW', 'IMPUTE', 'VALID HIGH', 'OUTLIER HIGH']
        ...
        var_ranges.rename(columns={c.replace(' ', '_'): n for c, n in to_rename.items()}, inplace=True)
    ...
    def filter_diagnoses_on_stays(self, diagnoses: pd.DataFrame, stays: pd.DataFrame) -> pd.DataFrame:
...
class ICUStay:
    ...
    def is_transfer_free(self) -> bool:
        return self.FIRST_CAREUNIT == self.LAST_CAREUNIT and self.FIRST_WARDID == self.LAST_WARDID
    ...
    def encode_ethnicity(self) -> int:
        ...
        def aggregate(ETHNICITY: str) -> str:
            return ETHNICITY.replace(' OR ', '/').split(' - ')[0].split('/')[0].strip().upper()
        
        return e_map.get(aggregate(self.ETHNICITY or ''), e_map['OTHER'])

    def encode_inunit_mortality(self) -> int:
        mortality: bool = False
        mortality = pd.notnull(self.DOD) and ((self.INTIME <= self.DOD) and (self.OUTTIME >= self.DOD))
        mortality = mortality or (pd.notnull(self.DEATHTIME) and ((self.INTIME <= self.DEATHTIME) and (self.OUTTIME >= self.DEATHTIME)))
    ...
    def filter_admissions_on_nb_icustays(cls, icustays_df: pd.DataFrame, min_nb_stays=1, max_nb_stays=1) -> List["ICUStay"]:
        ...
        to_keep = to_keep.loc[
            (to_keep.ICUSTAY_ID >= min_nb_stays) & 
            (to_keep.ICUSTAY_ID <= max_nb_stays)
        ][['HADM_ID']]
...
class Events(PandasMixin): 
    ...
    _cleaned_events: Optional[pd.DataFrame] = field(default=None, init=False) # cleaned events converted to time-series
    _timeseries: Optional[pd.DataFrame] = field(default=None, init=False) # time-series data indexed and sorted by CHARTTIME, there might be some duplicates due to some measurements being done at the same time
    ...
    def get_events(self) -> pd.DataFrame:
        event_header = ['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'CHARTTIME', 'ITEMID', 'VALUE', 'VALUEUOM']
        event_tables = []
    ...
    def get_phenotypes(self) -> pd.DataFrame:
        ...
        if self.phenotypes is None:
            ...
            diagnoses = self.data_manager.filter_diagnoses_on_stays(diagnoses, icustays_df)
            self.phenotypes = self.data_manager.make_phenotype_label_matrix(diagnoses, stays=icustays_df)
    ...
    clean_fns: ClassVar[Dict[str, Callable]] = {
        ...
        'Glucose': lambda df: Events.clean_lab(df), # rows with mimic label Fingerstick Glucose have NaN values in VALUEUOM however we have measurements for these in VALUE col
    ...
    def map_itemids_to_variables(self, events: pd.DataFrame) -> pd.DataFrame:
        ...
        mapped_events = events.merge(self.data_manager.variable_map, left_on="ITEMID", right_index=True, how="inner")
        if mapped_events.empty:
            unmapped = events["ITEMID"].unique()
            logger.warning(f"No ITEMIDs were successfully mapped to variables. Unmapped ITEMIDs: {unmapped}")
            return mapped_events
        self.events = mapped_events
        return self.events

    def remove_outliers_for_variable(self, events: pd.DataFrame, variable: str) -> pd.DataFrame:
    ...
    def impute_missing_values(self, events: pd.DataFrame, variable: str) -> pd.DataFrame:
        if variable not in self.data_manager.variable_ranges.index:
            return events
    ...
    def add_hours_since_admission(self, events: pd.DataFrame) -> pd.DataFrame:
        ...
        events = events[events["HOURS"] >= 0].copy() # filter out negative hours, recorded before admission (might have been an artifact, maybe a data entry delay or the patient was in emergency)
        return events
    
    def convert_to_timeseries(self, events: pd.DataFrame, variable_column="VARIABLE", variables=[]) -> pd.DataFrame:
        required_cols = ['CHARTTIME', 'ICUSTAY_ID', 'HOURS']
        if events.empty or not all(col in events.columns for col in required_cols):
            logger.warning("Events DataFrame is empty or missing required columns for timeseries conversion.")
            return pd.DataFrame()
        metadata = events[['CHARTTIME', 'ICUSTAY_ID', 'HOURS']].sort_values(by=['CHARTTIME', 'ICUSTAY_ID'])\
    ...
    def validate_events(self, events_df: pd.DataFrame, icustays_df: pd.DataFrame) -> bool:
        ...
        recovered = 0                  # empty ICUSTAY_IDs are recovered according to icu_stays (given HADM_ID)
        could_not_recover = 0          # empty ICUSTAY_IDs that are not recovered. This should be zero.
        icustay_missing_in_stays = 0   # ICUSTAY_ID does not appear in icu_stays, we exlude them
        ...
        merged_df = events_df.merge(icustays_df, left_on=['HADM_ID'], right_on=['HADM_ID'],
        ...
        final_df = merged_df[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'CHARTTIME', 'ITEMID', 'VALUE', 'VALUEUOM']]
    ...
    def get_cleaned_events(self) -> pd.DataFrame:
        if self._cleaned_events is None:
            ...
            for variable in cleaned_events["VARIABLE"].unique():
                cleaned_events = self.remove_outliers_for_variable(cleaned_events, variable)
            ...
            timeseries = self.convert_to_timeseries(cleaned_events, variable_column="VARIABLE", variables=cleaned_events["VARIABLE"].unique())
    ...
    def clean_sbp(df):
        ...
        variable.loc[idx] = variable[idx].apply(lambda s: re.match('^(\d+)/(\d+)$', s).group(1))
    ...
    def clean_dbp(df):
        variable = df["VALUE"].astype(str)
        idx = variable.apply(lambda s: '/' in s)
        variable.loc[idx] = variable[idx].apply(lambda s: re.match('^(\d+)/(\d+)$', s).group(2))
        return variable.astype(float)

    def clean_crr(df):
        """Clean capillary refill rate (CRR) values."""
        variable = pd.Series(np.nan, index=df.index)        
        df["VALUE"] = df["VALUE"].astype(str) # when df.VALUE is empty, dtype can be float and raises an exception, to fix this we change dtype to str
        variable.loc[(df["VALUE"] == 'Normal <3 secs') | (df["VALUE"] == 'Brisk')] = 0
        variable.loc[(df["VALUE"] == 'Abnormal >3 secs') | (df["VALUE"] == 'Delayed')] = 1
        return variable
    ...
    def clean_fio2(df):
        ...
        idx = df["VALUEUOM"].fillna('').apply(lambda s: 'torr' not in s.lower()) & (is_str | (~is_str & (variable > 1.0)))
        variable.loc[idx] = variable[idx] / 100.
        return variable
    ...
    def clean_lab(df):
        ...
        idx = variable.apply(lambda s: isinstance(s, str) and not (re.match(r'^(\d+(\.\d*)?|\.\d+)$', s)))
    ...
    def clean_o2sat(df):
        ...
        variable = df["VALUE"].copy()  
        idx = variable.apply(lambda s: isinstance(s, str) and not bool(re.match(r'^(\d+(\.\d*)?|\.\d+)$', s)))
        variable.loc[idx] = np.nan
    ...
    def clean_temperature(df):
        ...
        variable = df["VALUE"].astype(float)
        idx = df["VALUEUOM"].fillna('').apply(lambda s: 'F' in s.lower()) | df["MIMIC_LABEL"].apply(lambda s: 'F' in s.lower()) | (variable >= 79)
        variable.loc[idx] = (variable[idx] - 32) * 5. / 9
        return variable
...
class Patient(PandasMixin):
    subject_id: int
    data: Dict[str, pd.DataFrame] = field(default_factory=lambda: defaultdict(pd.DataFrame))
    icu_stays: List[ICUStay] = field(default_factory=list) # we might discard this since now we only do per hospital and remove multiple icu stays
    events: List[Events] = field(default_factory=list) # filtered, cleaned time-series events
    ...
    def add_icustay(self, icu_row: dict, merged_stays: pd.DataFrame): 
        try:
            ...
            assert(len(merged_stays['ICUSTAY_ID'].unique()) == len(merged_stays['ICUSTAY_ID']))            
            
            patient_data = merged_stays.loc[merged_stays["SUBJECT_ID"] == self.subject_id]
            ...
            logging.warning(f"Failed to parse ICU stay for subject {self.subject_id}: {e}")
    ...
    def filter_diagnoses_on_stays(diagnoses, icustays_df) -> pd.DataFrame:
        ...
        return diagnoses.merge(icustays_df[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID']].drop_duplicates(), how='inner',
    ...
    def process_patient(pid_patient_tuple):
        ...
        if len(patient.icu_stays) > 1:
            ...
            logger.warning(f"Check multiple stays for the same admission: Patient {pid} has {len(patient.icu_stays)} ICU stays: HADM_IDs {hadm_ids}")
            logger.warning(f"After filtering by ICU stay count, {len(stays)} stays remain for Patient {pid}")
        ...
        logger.warning(f"Removed {len(removed_due_to_age)} patients due to age restriction")
        logger.warning(f"Removed {len(removed_due_to_transfer)} patients due to transfer restriction")
        logger.warning(f"After applying age and transfer filters, {len(stays)} stays remain for Patient {pid}")
        ...
        if len(stays) == 1: 
            ...
            logger.info(f"Patient {pid} has exactly one valid ICU stay, keeping the patient.")
            return (pid, patient)
        else:
            logger.warning(f"Patient {pid} has {len(stays)} ICU stays, excluding from final list.")
            return None

    @classmethod
    def build_patient_database(cls, tables: Dict[str, pd.DataFrame], filter_single_stay: bool = True, csv_path: Optional[str] = None) -> Tuple[Dict[int, "Patient"], Dict[int, "Patient"]]:
        ...
        filtered_patients = {pid: patient for result in results if result is not None for pid, patient in [result]}
